...
public class MappedFile extends ReferenceResource {
    ...
    public static void ensureDirOK(final String dirName) {//确保路径存在
        if (dirName != null) {
    ...
    public void init(final String fileName, final int fileSize,
        final TransientStorePool transientStorePool) throws IOException {
        init(fileName, fileSize);
        this.writeBuffer = transientStorePool.borrowBuffer();//从堆外内存获取一块写缓存
    ...
    private void init(final String fileName, final int fileSize) throws IOException {
        ...
        this.fileFromOffset = Long.parseLong(this.file.getName());//文件名就是消息的偏移量
        boolean ok = false;
    ...
    protected void commit0(final int commitLeastPages) {
        ...
        if (writePos - this.committedPosition.get() > 0) {//writeBuffer的缓存大小跟文件大小相同，需要截取出未刷新的内容
            try {
    ...
    private boolean isAbleToFlush(final int flushLeastPages) {
        ...
        if (flushLeastPages > 0) {//缓存的数据（按页算）是否超过了给定的页数
            return ((write / OS_PAGE_SIZE) - (flush / OS_PAGE_SIZE)) >= flushLeastPages;
        }

        return write > flush; //没指定页数时，是否有缓存数据
    }
    ...
    protected boolean isAbleToCommit(final int commitLeastPages) {
        ...
        if (commitLeastPages > 0) {//堆外内存缓存的数据(按页算)是否超过了给定的页数
    ...
    public SelectMappedBufferResult selectMappedBuffer(int pos, int size) {//截取原buffer中从pos开始，大小为size的缓冲区映射
    ...
    public SelectMappedBufferResult selectMappedBuffer(int pos) {//截取原buffer中从pos开始，到当前最大读位置的数据缓冲区映射
        int readPosition = getReadPosition();
        if (pos < readPosition && pos >= 0) {//获取的位置在可读范围内
            if (this.hold()) {//引用计数加1
                ByteBuffer byteBuffer = this.mappedByteBuffer.slice();
                byteBuffer.position(pos);
     ...
     */
    public int getReadPosition() {//如果有堆外缓存，则取堆外缓存已经提交的最大的位置；否则使用已经mappedbuffer的最大值
    ...
    public void warmMappedFile(FlushDiskType type, int pages) {//写入pages页数据，值为0
        long beginTime = System.currentTimeMillis();
        ByteBuffer byteBuffer = this.mappedByteBuffer.slice();
        int flush = 0;
