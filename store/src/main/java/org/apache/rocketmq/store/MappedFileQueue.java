...
public class MappedFileQueue {
    ...
    /**
     * 将偏移量所在区间的文件块的起始位置重置，将偏移量后面的文件块从内存中释放
     */
    ...
    /**
     * 将文件夹下的文件按照文件名排序，然后加载进来
     * 设定写入位置的偏移量；刷新位置的偏移量；提交位置的偏移量均为文件大小
     */
    public boolean load() {
        ...
        try {
            ...
            if (firstMappedFile != null && lastMappedFile != null) {
                if (offset < firstMappedFile.getFileFromOffset() || offset >= lastMappedFile.getFileFromOffset() + this.mappedFileSize) {
                    ...
                    //根据文件大小计算出给定偏移量所在文件的位置，使用下标返回对应的文件，判断该文件是否符合；否则从头遍历所有的文件，找出符合的文件
