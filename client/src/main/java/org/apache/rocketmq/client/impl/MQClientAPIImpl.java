...
public class MQClientAPIImpl {
    ...
    private final RemotingClient remotingClient;//netty rpc客户端实现
    ...
    /**
     * 从http://xxxx:port/rocketmq/xxx，默认8080端口中获取nameserver地址
     * 返回结果为一个ip列表，以;隔开
     * 如果获取回来的地址跟现有的地址不一致
     * 更新缓存的namesrv地址列表
     *
    ...
    public SendResult sendMessage(
        ...
        if (sendSmartMsg || msg instanceof MessageBatch) {//判断是否为批处理消息，设置不同的请求头
    ...
    /**
     * 构造请求头，调用netty客户端发送
     * 选择一个已经同namesrv建立连接的channel，发起请求， addr传null
     */
    public TopicRouteData getTopicRouteInfoFromNameServer(final String topic, final long timeoutMillis,
