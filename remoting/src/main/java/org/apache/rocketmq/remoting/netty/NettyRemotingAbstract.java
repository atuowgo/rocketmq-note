...
public abstract class NettyRemotingAbstract {
        ...
        if (cmd != null) {
            switch (cmd.getType()) {
                case REQUEST_COMMAND:
                    ...
                    break;
                case RESPONSE_COMMAND://客户端只会有这个
    ...
    public void processRequestCommand(final ChannelHandlerContext ctx, final RemotingCommand cmd) {
        ...
        if (pair != null) {//可处理请求
            Runnable run = new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        //如果是单向请求，不返回结果
                        if (!cmd.isOnewayRPC()) {
                            //如果返回结果不为null，则认为要让框架flush返回;否则不会主动flush
                            if (response != null) {
            ...
            if (pair.getObject1().rejectRequest()) {//请求被拒绝，返回系统繁忙
    ...
    /**
     * 通过semaphore信号量控制最多有多少个连接
     * 请求发起后，将结果对象缓存起来
     * 结果将通过InvokeCallback进行回调
     * 如果有设置回调函数，结果返回，在回调线程发起后就会将信号量回收
     * 如果没有设置回调函数，结果返回后就会将信号量回收
     */
